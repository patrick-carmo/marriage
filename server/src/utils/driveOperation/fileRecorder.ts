...
const saveImageData = async (data: ImagesData, user_id: number, folder_id: string) => {
  await knex.transaction(async trx => {
    const image_folder = await saveImageFolder(user_id, folder_id, trx)
    await trx('images').insert({ image_id: data.image_id, url: data.url, user_id, image_folder })
  })
}

const saveImageFolder = async (user_id: number, folder_id: string, trx: any): Promise<number> => {
  const existingFolder = await trx('image_folder').where({ user_id }).first()
  if (!existingFolder) {
    const newFolder = await trx('image_folder').insert({ folder_id, user_id }).returning('id')
    return newFolder[0].id
  }
  await trx('image_folder').where({ user_id }).update({ folder_id })
  return existingFolder.id
}
